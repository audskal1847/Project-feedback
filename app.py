...
# 2. 사이드바: API 제공자 및 키 설정 (요청 모델 라인업 고정 및 추가)
with st.sidebar:
    st.header("🔑 API 설정")
...
# 5. 기존(사전) 활동 입력 및 파일 업로드 기능 (기능 확장)
...
3. [탐구 과정 가이드 (상세화)]
   ...
   - [동기 및 가설 설정]: 사전 활동의 의문점이나 교과서 개념에서 출발하여 어떻게 이 주제로 넘어왔는지, 어떤 가설을 세울 수 있는지 (개조식)
...
# 10. 프로그램 하단 만든 이 및 제목 표시 푸터 (Footer 신설)
st.markdown("---")
st.markdown(
    """
    <div style="text-align: center; color: #7f8c8d; font-size: 0.9em; line-height: 1.6;">
        <strong>🏫 탐구 역량 강화를 위한 주제 탐구 구체화 어시스트 프로그램 v2</strong><br>
